...
def search_orders_by_customer(customer_name: str) -> list[dict]:
    ...
    return [o for o in json.loads((ROOT / 'data/orders.json').read_text()) if customer_name.lower() in o['customer_name'].lower()]
def calculate_order_total(order_id: str) -> float:
    """Return the recorded AUD total for one synthetic order."""
    return next((o['total_aud'] for o in json.loads((ROOT / 'data/orders.json').read_text()) if o['order_id'] == order_id), 0.0)
...
async def main() -> None:
    ...
    order = json.loads((ROOT / 'data/orders.json').read_text())[0]
