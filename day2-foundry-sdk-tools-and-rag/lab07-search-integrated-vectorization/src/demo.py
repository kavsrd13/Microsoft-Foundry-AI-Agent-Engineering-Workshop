...
def main() -> None:
    ...
    for chunk in json.loads((ROOT / 'data/chunks.json').read_text(encoding='utf-8')):
    ...
    print(name, 'configured. Indexer runs on creation; inspect execution status using python validate.py --live.')
    print('For later changed input, run: python src/query.py --run-indexer')
