...
def main() -> None:
    ...
    with DefaultAzureCredential() as credential, AIProjectClient(
        ...
        state = root / 'response-ids.json'
        ids = json.loads(state.read_text(encoding='utf-8')) if state.exists() else []
        ids.append(response.id)
        (root / 'response-ids.json').write_text(json.dumps(ids), encoding='utf-8')
        ...
        (root / 'live-output.txt').write_text(response.output_text, encoding='utf-8')
