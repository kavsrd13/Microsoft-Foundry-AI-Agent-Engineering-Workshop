...
3. From the lab directory create a Python 3.13 environment, activate it, and install `requirements.txt`. Then `cd src/deploy`. This folder is a self-contained azd project; its `agent/main.py` is the same simple host concept.
...
Cleanup: from this exact `src/deploy` directory, select the recorded dedicated lab environment and review `azd env get-values` locally. Then use `azd down` and its confirmation to remove only that environment's resources. Never run down against a shared classroom environment. Recheck the portal for the recorded resource group/agent. Stop local servers with Ctrl+C and run `python cleanup.py` from the lab folder for local generated files.
