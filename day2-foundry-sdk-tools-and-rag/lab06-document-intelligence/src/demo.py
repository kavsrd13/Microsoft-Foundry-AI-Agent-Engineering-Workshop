...
def main() -> None:
    ...
    chunks = []
    for pdf in sorted((ROOT / 'data/benefits').glob('*.pdf')):
        ...
        (ROOT / 'data' / (pdf.stem + '.md')).write_text(result.content, encoding='utf-8')
    ...
    (ROOT / 'data/chunks.json').write_text(json.dumps(chunks, indent=2), encoding='utf-8')
