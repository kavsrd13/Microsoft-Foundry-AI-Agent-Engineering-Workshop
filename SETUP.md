...
Then open Lab 01 in its own folder, populate `.env`, and run `python -m compileall .`. Offline syntax and asset checks do not prove Entra access, model availability, indexing, evaluator availability or hosted deployment. Run each live walkthrough in the actual training tenant before delivery.

Optional Microsoft samples: `python shared/download_ms_samples.py`. Review upstream licence terms first. Invoice_1.pdf is a local alias for the maintained upstream invoice/invoice.pdf because the requested original binary is absent. Synthetic PDFs remain the default.

## Client labs17–24

Follow the [client course route](CLIENT-COURSE-MAP.md) and each lab's own requirements and `the shared workshop .env`. Use isolated environments. Lab22 reuses Lab21 application source. Lab21 includes a dedicated GUID-based index seed; Lab23 requires the original Lab08 corpus/permission schema. Lab18's small retrieval experiment uses a separate index and cannot supply a release-gate report. Tenant identities, role assignments, governance services and channel licences require instructor preparation.
