...
class Preferences(ContextProvider):
    async def before_run(self, *, agent: SupportsAgentRun, session: AgentSession, context: SessionContext, state: dict[str, Any]) -> None:
        profiles = json.loads((ROOT / 'user_preferences.json').read_text())
...
async def main() -> None:
    ...
    async with DefaultAzureCredential() as credential:
        ...
        async with client:
            ...
            (ROOT / 'session.json').write_text(json.dumps(session.to_dict(), indent=2))
            restored = AgentSession.from_dict(json.loads((ROOT / 'session.json').read_text()))
