...
- Bicep CLI **0.47.16** built `infra/main.bicep` including `resources.bicep` with exit 0 and no diagnostics.
- `compileall` passed service-path, parameter-JSON, workflow gate-order and basic scaffold checks.
- Lab21 `compileall` passed local RSA-signed JWT verification/rejection tests, missing/forged-header checks, ACL filter checks and mocked SSE completion.
