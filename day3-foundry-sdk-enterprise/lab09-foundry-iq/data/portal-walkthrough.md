...
9. Delete only the recorded lab09 knowledge base, knowledge source, index and optional Blob container in reverse order through the portal. Recheck each exact name before deletion. The Python scripts do not create these portal resources, so `cleanup.py` only removes Python-owned outputs. Retain shared services and project connections.
