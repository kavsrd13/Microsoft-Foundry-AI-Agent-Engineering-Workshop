...
def main() -> None:
    ...
    rows = [json.loads(line) for line in (root / 'data' / 'qa_dataset.jsonl').read_text(encoding='utf-8').splitlines()]
    ...
    with AIProjectClient(endpoint=os.environ['PROJECT_ENDPOINT'], credential=credential) as project, project.get_openai_client() as client:
        for number, row in enumerate(rows, 1):
            ...
            (root / 'data' / 'scores.json').write_text(json.dumps(results, indent=2), encoding='utf-8')
            (root / 'data' / 'score-report.md').write_text('\n'.join(report), encoding='utf-8')
    credential.close()
