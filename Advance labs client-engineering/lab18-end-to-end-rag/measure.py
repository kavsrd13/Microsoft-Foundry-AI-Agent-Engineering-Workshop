...
dataset = (ROOT / 'questions.json').read_bytes()
...
for mode in ['keyword', 'vector', 'hybrid', 'semantic']:
    ...
    (ROOT / f'retrieval-{mode}.json').write_text(json.dumps(report, indent=2))
    if mode == 'semantic':
        (ROOT / 'retrieval-report.json').write_text(json.dumps(report, indent=2))
